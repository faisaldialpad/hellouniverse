...
import trees_graphs.InOrderBSTIterator;
import trees_graphs.TNode;

public class InOrderBSTIteratorTest {

	@BeforeClass
	public static void setUpBeforeClass() throws Exception {
		assertNotNull(new InOrderBSTIterator(null));
	}
	...
	public void testNormal() {
		...
		TNode<Integer>previous;
		TNode<Integer>current;
		do{
			previous=it.getCurrent();
			current=it.next();
			if(previous!=null && current!=null){
				assertTrue(previous.data<current.data);
			}
		}while(current!=null);
	}
